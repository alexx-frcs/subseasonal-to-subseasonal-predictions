...
def lasso_fit_and_predict(df, x_cols=None, base_col=None, clim_col=None,
    ...
    return pd.DataFrame(
        {'pred': fit_model.predict(test_df[x_cols]) + test_df[base_col].values - test_df[clim_col].values,
         ...
         'clim': test_df[clim_col].values},
        index=[test_df.lat,test_df.lon,test_df.start_date])
def xgboost_regression(X, y, t, threshold_date, ridge=0.0):
        # Extract size of feature matrix
    model=xgb.XGBRegressor()
    (n, p) = X.shape
    model.fit(X,y)
    
#regarder ce qu'il se passe avec les dates, pour savoir à partir de quelles dates faire les prédictions
#regarder comment fonctionne la fonction rolling_linear_regression pour savoir justement quelles dates considérer
#vérifier la nature des inputs (une date ou un groupe de dates à chaque fois)
def xgboost_regression_wrapper(df, x_cols=None, base_col=None, clim_col=None, anom_col=None, 
    last_train_date=None,ridge=0.0):
    
    train_df = df.dropna(subset=x_cols+['target'])
    preds = xgboost_regression(train_df[x_cols], 
                                      train_df['target'], 
                                      train_df.start_date, 
                                      last_train_date, 
                                      ridge = ridge)
...
def backward_rolling_linear_regression_wrapper(
    ...
    # and set index
    base_minus_clim = train_df[base_col].astype(float).values - train_df[clim_col].astype(float).values
