...
def get_similar_years_hindcast(gt_col, target_horizon, target_date = pd.to_datetime("1999-04-18"),
    ...
    for yr in range(first_year, 2019 if hindcast else target_date.year + 1):
        ...
        # Only proceed if all margin_days days are available
        if df_yr.start_date.unique().size == margin_days:
            key_cols = [col for col in df_yr.columns if col.startswith('key_')]
            df_yr = df_yr.drop(columns=key_cols)
            key_cols = [col for col in df_target_yr.columns if col.startswith('key_')]
            df_target_yr = df_target_yr.drop(columns=key_cols)
...
"""
la fonction suivante renvoie un dictionnaire pour lequel chaque clé (par exemple 'mean' qui est le critère qui nous intéresse) contient un dataframe contenant les cosinus pour toutes les colonnes de x_cols_current
"""
