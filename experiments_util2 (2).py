...
def get_measurement_variable(gt_id, shift=None):
    ...
    if "wind" or "GPP" or "RECO" or "CASM" or "elevation" in gt_id:
        return gt_id+suffix
...
def get_ground_truth(gt_id, multiflux_resolution = "monthly", mask_df=None, shift=None):
    ...
    if gt_id.startswith("GPP") or gt_id.startswith("RECO") :
        ...
        gt=pd.read_csv('data/new features/casm_us_sm.csv')
        ...
        gt = shift_df_new(gt,shift=44, date_col='date', groupby_cols=['lat', 'lon'])
...
def get_date_CASM(first_year=None, mask_df=None):
    #ds = xr.open_dataset('gs://leap-persistent/data-library/casm-595733423-4997696883-1/CASM.zarr', engine='zarr', chunks={}) 
    df=pd.read_csv('data/new features/casm_us_sm.csv')
    df['date']=pd.to_datetime(df['date'])
    #gt = ds.CASM_soil_moisture.date #remove .time for 3D
    #gt=pd.DataFrame(gt)
    return df
...
##add elevation 
    ...
    t = time.time()
    #lat_lon_date_data.to_hdf(lat_lon_date_data_file, key="data", mode="w",format='table')
...
def create_lat_lon_data(gt_id,
    ...
    if lat_lon_features:
        ...
        #lat_lon_data.to_hdf(lat_lon_data_file, key="data", mode="w",format='table')
...
def create_date_data(gt_id,
    ...
    t = time.time()
    #date_data.to_hdf(date_data_file, key="data", mode="w",format='table')
...
def create_sub_data(gt_id,
   ...
   # sub_data.to_hdf(sub_data_file, key="data", mode="w",format='table')
